 ...
 * download (attachment disposition — triggers browser save dialog).
 *
 * SRP: single responsibility — only orchestrates the download fetch.
 * DIP: depends on the FileRepository interface (domain abstraction),
 *      never on the concrete HTTP implementation.
 *
 * @example
 * const useCase = new DownloadFileUseCase(new FileRepositoryImpl());
 * const blob    = await useCase.execute('connections', 'contrato.pdf');
 */
export class DownloadFileUseCase {
  ...
  async execute(type: FileCategory, filename: string): Promise<Blob> {
    ...
    return this.repository.download(type, filename);
  }
}
