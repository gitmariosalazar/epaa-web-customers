...
interface MonthlyPaymentMethodsTableProps {
  data: PaymentMethodSummary[];
  isLoading: boolean;
  year: number;
}

export const MonthlyPaymentMethodsTable: React.FC<
  MonthlyPaymentMethodsTableProps
> = ({ data, isLoading, year }) => {
  const { setShowPdfPreview, PdfPreviewModal } =
    useTablePdfExport<PaymentMethodSummary>({
      ...
      reportTitle: 'Resumen de Métodos de Pago - Convenios (Mensual)',
      reportDescription: `Distribución por tipo de cobro - Año ${year}`,
      mapRowData: (row) => [
        row.paymentMethod,
  ...
  const columns: Column<PaymentMethodSummary>[] = useMemo(
    () => [
    ...
    >
      <div className="overdue-chart-header">
        ...
        <p className="overdue-chart-subtitle">
          Distribución por tipo de cobro en {year}
        </p>
      </div>
      <div
