...
export class ReceiveWorkOrderUseCase {
  ...
  async execute(
    receiveWorkOrderCommand: ProcessWorkOrderRequest
  ): Promise<ProcessWorkOrderResponse | null> {
    const result = await this.processWorkOrderRepository.receiveWorkOrder(
